...
psychologist_router.callback_query.middleware(SubMiddleware())
psychologist_router.message.middleware(SecondSubMiddleware())
...
@psychologist_router.callback_query(F.data.startswith('get_psychologist'))
async def command_start_handler(call: CallbackQuery, state: FSMContext) -> None:
    if len(call.data.split(':')) == 1:
        msg = await edit_message(call.message, """На обучении есть психолог, который разберет каждый твой запрос, если будешь сталкиваться со сложностями во время обучения. 

Отправь сообщение, чтобы психолог связался с тобой""", reply_markup=keyboard.psychologist_keyboard(call.from_user.id), parse_mode="HTML")
    else:
        try:
        ...
        msg = await call.message.answer("""На обучении есть психолог, который разберет каждый твой запрос, если будешь сталкиваться со сложностями во время обучения. 
...
@psychologist_router.message(StateFilter(SendPsycho.psycho))
async def command_start_handler(message: Message, state: FSMContext) -> None:
    ...
    try:
        msg_1 = await message.bot.send_message(config.PSYHOLOGIST_CHAT_ID, f'⬇️ {message.from_user.full_name} @{message.from_user.username} ({user_data[0]["email"].lower()} Поток: {users_flow}) отправил сообщение (Техническая информация: {message.from_user.id}) ⬇️', reply_markup=keyboard.web_app_psychologist_chat_keyboard(message.from_user.id))
        await message.bot.forward_message(config.PSYHOLOGIST_CHAT_ID, message.chat.id, message.message_id)
    except Exception:
        print(traceback.format_exc())
        pass

    msg = await message.answer('Сообщение отправлено, психолог свяжется с тобой в течение 2-ух дней', reply_markup=keyboard.psychologist_keyboard(message.from_user.id))
    await state.update_data(message_id=msg.message_id)
    db.add_to_psychologist_messages(message.from_user.id, message.chat.id, message.html_text, file_id, file_type, True, time.time(), None if msg_1 is None else f"https://t.me/c/{-(msg_1.chat.id+1000000000000)}/{msg_1.message_id}")

    if str(message.from_user.id) in config.ws_connections_psychologist:
        message_payload = {
            "type": "message",
        ...
        for ws in config.ws_connections_psychologist[str(message.from_user.id)]:
